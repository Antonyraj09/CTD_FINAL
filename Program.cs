...
// tenant on every request — see TenantCookieValidator / AccountController.Login for
...
builder.Services.AddScoped<IJobMisctService, JobMisctService>();
...
builder.Services.ConfigureApplicationCookie(options =>
{
    ...
    // Overrides AddIdentity's default OnValidatePrincipal (SecurityStampValidator.ValidateAsync),
    // which fires inside UseAuthentication() on every request carrying a valid cookie — before
    // UseTenantResolution() would otherwise get a chance to run. Merely resolving the default
    // security-stamp validator via DI needs AppDbContext, so every authenticated request was
    // failing. TenantCookieValidator resolves the tenant from the LicenseNumber claim first
    // (AdminDbContext only), then delegates to the normal security-stamp check once that's safe.
    options.Events.OnValidatePrincipal = TenantCookieValidator.ValidateAsync;
});
