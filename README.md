...
- **Importer Code**: a fixed `NP0000` prefix (shown as a non-editable label) plus an editable 4-digit numeric suffix — the two pieces are combined into one 10-character value (`JobIsne.ImporterCode`, `RegularExpression(@"^(NP0000\d{4})?$")` — matches empty or a full valid code) both client-side (`#isne_importerCodeSuffix` strips non-digits and caps at 4 as you type, same convention as CTD/Container Number) and server-side (`JobIsneController.Save`, since the client filter can be bypassed). Editing an existing job splits the stored value back into just the 4-digit suffix for display; leaving the suffix blank stores no Importer Code at all rather than a malformed `NP0000` fragment.
