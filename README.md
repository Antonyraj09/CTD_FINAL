...
`JobIsneController.CtdDeclaration(id)` / `Views/JobIsne/CtdDeclaration.cshtml` is a new report: the official CHA-issued "Customs Transit Declaration (Import) ICCD" form, replicated as a bordered grid layout matching the reference paper format field-for-field, opened for browser printing (`window.print()`, same convention as every other print view — print-to-PDF is the browser's own dialog, no server-side PDF generation). Available from both the Job ISNE screen and a CTD Tracking row action, alongside (not replacing) the checklist. Its grid was rebuilt against the reference PDF's exact vector geometry (`page.get_drawings()` line/rect coordinates, not just the rendered image), because several of its sections split asymmetrically between the left and right columns — e.g. "Importer's Name & Address" is one tall left cell spanning the same height as two stacked right cells ("Verification" then "Country of Consignment"); "HMO' & Import Licence No. and Date" is one tall right cell spanning "Notify or delivery address" and "Statement as to transportation" on the left — so the HTML uses explicit extra `<tr>` rows with `rowspan` on the less-subdivided column to reproduce that, and likewise the footer's "Declaration" + signature cell carries `rowspan="2"` to span both "Endorsement by Customs House on entry" and "Endorsement by Indian Border Customs Office on Exit". The "Org/Dup/..." copy-type line was moved into the title column (it sits above "CUSTOMS TRANSIT DECLARATION" in the reference, not inside the CHA header block where it had been misplaced), and the cargo grid is four true columns (Marks / Description / Gr.&Net Wt. / Cif&Market Value, each width matched to the reference's point coordinates) instead of a merged weight-value pair. That vector-geometry pass was later cross-checked against a `.docx` version of the same reference form — reading its `word/document.xml` table directly (`gridCol` widths, `gridSpan`/`vMerge` per cell) gives the exact column-width and merge blueprint unambiguously, no coordinate math needed, and it caught two structural gaps the PDF-only pass missed: "Notify or delivery address"/"Same as above" and "Statement as to transportation"/"By Road/Rail" are each two separate side-by-side cells (not one cell with the label and value stacked), and "Ship's Name"/"Rotation No."/"Line No."/"B/L No. & Date" is a label row followed by a separate value row (not label-and-value stacked in one cell) — the view's single main `<table>` now carries an explicit 9-column `<colgroup>` (percentages taken straight from the `.docx` `gridCol` widths) with `colspan`/`rowspan` on each cell matching the reference table's `gridSpan`/`vMerge` exactly, so mixed-width rows can't drift out of alignment with each other under `table-layout: fixed`. It's since gone through one more precision pass, using dimensions read directly from the reference `.docx`'s `sectPr` (page size/margins), `tblGrid` (column widths) and each row's `trHeight` (`hRule="exact"`) rather than percentages/estimates: `@@page` uses the exact A4 margins from `pgMar` (3.88mm/9.98mm/4.94mm/2.49mm), the 9-column `colgroup` uses the `gridCol` widths converted straight to millimetres instead of rounded percentages, and every `<tr>` carries an explicit `height` converted from the reference's own `trHeight` (twips → mm) — because the reference is one single, uninterrupted `<w:tbl>` covering the whole form (no gaps between sections), the view is now one continuous `<table>` too, not four separate boxes with margins between them. The cargo grid's "Marks and Serial No. of Pkgs" cell carries `rowspan="2"` over the header + data rows, same as the reference, which is what gives it its correct ~111.6mm height (the tallest section on the page, by design — a full CIF/duty/FOB breakdown has to fit in that one cell) rather than the visually-squashed version the percentage-only pass produced. Row/column sizes are fixed to these exact reference dimensions regardless of whether a given job's data is short or empty — this is a customs-submission form, not a reflowing report, so its box structure has to match the paper form printed-page-for-printed-page; a `height` on a table row is a floor, not a clip, so unusually long data (e.g. a very long address) still grows the row rather than being cut off, which matters more for a legal filing than millimetre-perfect matching on the rare oversized value. The Gr./Net Weight and Cif/Market Value sub-groups within the (now very tall) weight/value columns use a fixed-height flex column with a `margin-top` spacer between the two label/value pairs (matching the reference's own vertical spacing) instead of a nested table with percentage row heights, which browsers don't reliably size against a table-cell ancestor's height. Base font is Arial at 10pt, uniformly — every per-class `font-size` override (labels, titles, table headers, declaration text) was removed so all text inherits the same 10pt rather than the mix of 8.7–15px sizes used before, and bold is now reserved strictly for database-fetched values: static labels, headings and boilerplate text (`Verification`, `Declaration,`, the copy-type line, table column headers, `Same as above`, `By Road/Rail`, `In Transit To Nepal`) are regular weight, while every actual `JobIsne`/`SubAgent` field — including ones embedded mid-sentence like the CHA No. after "C.H.Aagent No." or the numbers inside the FOB/Freight/Insurance breakdown — is wrapped in `<b>` (or a `font-weight:700` class) individually rather than bolding the whole containing line. Uniformly bumping every label to 10pt (up from the reference's own smaller 8–9pt label sizes) ate the vertical slack the fixed reference row heights were originally sized around, so the page started spilling one line onto a second sheet; recovered it with a body-wide `line-height: 1.15` (tighter than the default) and by shrinking the trailing "Printed from..." caption, which isn't part of the customs form itself, down to 7pt — the whole submittable form still renders on exactly one A4 page for the reference's own data. A real CHA agency name/address can run considerably longer than the reference sample, though, which pushes the header row (and, since the same long name repeats in the footer's "For ...", the footer row too) past their reference-derived floor heights enough to spill a second page — verified with a stress test using a ~90-character agency name. Every `<tr>` now carries `break-inside: avoid` / `page-break-inside: avoid` so if a row genuinely needs more room than fits, Chrome moves the whole row to the next page instead of slicing it mid-cell (which is what happened before — a table row split with its top half on page 1 and bottom half on page 2, far worse for a signature block than a clean page break). Padding and a few fixed margins (`title-body`, `copytype`, `sign-block`, `container-block`) were also trimmed and the base `line-height` tightened to 1.1 to reclaim enough slack that the reference's own data, and moderately long real names, stay on one page; only pathological cases (a company name close to 100 characters) still spill, and they now do so cleanly rather than breaking mid-row. The CHA header cell's three lines were re-checked paragraph-by-paragraph against the reference `.docx` (not just visually) and corrected to match exactly: line 1 is the agency name with the C.H. Agent No. right-floated so it sits at the top-right and the (long) name wraps around it, matching a Word tab stop that runs out of room, rather than the name and CHA No. stacked on separate lines; line 2 is the full address as one continuous line (not broken into separate address/city lines) ending with the "D" copy-type marker, since the reference literally has it there, not on its own line; line 3 is the phone number immediately followed by "GSTIN ..." with no separator at all, because that's what the reference paragraph itself contains (`<w:t>2243-5348</w:t><w:t>GSTIN</w:t>` back to back, no space or tab run between them) — reproduced as-is rather than "fixed" with a space, per the standing instruction to output exactly what the reference gives. Field mapping: CHA header block (name, C.H. Agent No., address, phone, GSTIN) ← a `SubAgent` lookup by `JobIsne.SubAgentCode` (not just the free-text `SubAgentName` JobIsne itself carries); Importer's Name & Address block now also prints `JobIsne.ImporterCode` as the "EXIM:" line beneath the address, matching the reference (this field existed on the entity but had been omitted from the view — the reference PDF shows it and the instruction was "don't miss single field"); HMO' & Import Licence box ← Bank Name + LC No/Date; Ship's Name ← Vessel Name + Voyage No; B/L No. & Date ← Master B/L, falling back to House B/L only if no Master B/L is on file (confirmed against the CTD Submission checklist's reference format, which shows the Master B/L number in this exact slot); the cargo grid's per-container "Marks and Serial No." and container-size/package lines iterate `JobIsneContainer`, while Gr./Net Weight, CIF and Market Value come from JobIsne's own job-level fields; the FOB/Freight/Insurance/CIF computation block is formatted to match the reference form's exact wording, INR values grouped Indian-style (lakhs/crores) via the `en-IN` culture. The Verification and Declaration paragraphs are reproduced character-for-character from the reference, including its own apparent typos ("In thie Declaration", "spcified", "Hcence", "entires", "and. belief", "Authorised SIgnatory") — per the explicit instruction to output exactly what the reference gives, not a corrected version. "Statement as to transportation," the "Org/Dup/..." copy-type line, and "Notify or delivery address" have no JobIsne equivalent and are rendered as static text, same blank-fill-in convention as the checklist.
